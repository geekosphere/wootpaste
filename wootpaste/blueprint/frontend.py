...
def paste_create():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if config['paste.spam_ml'] and not paste.private and not g.user:
            if spam_ml.predict(form.content.data) == spam_ml.SPAM:
                paste.spam = True
                db_session.commit()
                raise SpamDetected()
        db_session.commit()
...
def paste_index():
    pastes = Paste.query.\
        filter_by(private=False, encrypted=False).\
        filter((Paste.spam == None) | (Paste.spam == False)).\
